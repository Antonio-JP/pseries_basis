...
class OrthogonalBasis(PolyBasis):
    ...
    def __compatibility_derivation(self, pow_Q, pow_D):
        Q,L,l = self.get_differential_equation();
        if(pow_Q < pow_D):
        ...
        if(n == 1):
            try:
                ...
                Q2, a,b,c = self.get_mixed_equation();
                N = self.n; Sn = self.Sn;
...
class HermitePBasis(OrthogonalBasis):
    r'''
        ...
        This class represents the OrthogonalBasis formed by the set of probabilsitic Hermite polynomials
